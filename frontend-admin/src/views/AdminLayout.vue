  ...
  <el-container class="layout">
    <el-aside width="260px" class="aside">
      <div class="brand">
        ...
        <div class="brand-text"><div class="brand-name">XHXT</div><div class="brand-sub">管理中心</div></div>
      </div>
      <el-menu class="menu" :default-active="activeMenu" router background-color="transparent" text-color="#8b92a9" active-text-color="#00d4ff">
        <el-menu-item index="/admin" class="menu-item"><span class="menu-icon">📊</span><span>首页概览</span></el-menu-item>
        <el-menu-item index="/admin/users" class="menu-item"><span class="menu-icon">👥</span><span>用户与权限</span></el-menu-item>
        <el-menu-item index="/admin/products" class="menu-item"><span class="menu-icon">📦</span><span>商品中心</span></el-menu-item>
        <el-menu-item index="/admin/content" class="menu-item"><span class="menu-icon">📝</span><span>内容与社区</span></el-menu-item>
        <el-menu-item index="/admin/orders" class="menu-item"><span class="menu-icon">🧾</span><span>订单列表</span></el-menu-item>
        <el-menu-item index="/admin/stats" class="menu-item"><span class="menu-icon">📈</span><span>财务与数据</span></el-menu-item>
        <el-menu-item index="/admin/profile" class="menu-item"><span class="menu-icon">⚙️</span><span>个人中心</span></el-menu-item>
      </el-menu>
    </el-aside>

    <el-container>
      <el-header class="header">
        <div class="header-left"><span class="header-title">管理后台</span></div>
        <div class="header-right">
          ...
          <div class="user-info" v-if="user">
            ...
            <div class="user-details"><div class="user-name">{{ user.nickname || user.account }}</div><div class="user-role">{{ user.role === 'ADMIN' ? '管理员' : '员工' }}</div></div>
          </div>
...
async function loadNotify() {
  const res = await http.get('/admin/order/notify/list')
...
async function markRead(row) {
  await http.post(`/admin/order/notify/read/${row.orderNo}`)
  await loadNotify()
}
...
.aside { background: linear-gradient(135deg, #1e3c72 0%, #2a5298 100%); color: #fff; overflow-y: auto; box-shadow: 2px 0 8px rgba(0, 0, 0, 0.15); }
.brand { display: flex; align-items: center; gap: 12px; padding: 24px 16px; border-bottom: 1px solid rgba(255, 255, 255, 0.1); }
.brand-icon { font-size: 32px; }
.brand-name { font-weight: 700; font-size: 16px; color: #fff; }
.brand-sub { font-size: 12px; color: rgba(255, 255, 255, 0.7); margin-top: 2px; }
.menu { border-right: none; background: transparent; padding: 12px 0; }
.menu-item { display: flex; align-items: center; gap: 10px; margin: 4px 12px; border-radius: 8px; transition: all 0.3s ease; }
...
.user-avatar { width: 40px; height: 40px; border-radius: 50%; background: linear-gradient(135deg, #1e3c72 0%, #2a5298 100%); color: #fff; display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 16px; }
